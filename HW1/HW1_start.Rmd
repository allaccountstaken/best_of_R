...
belong to. Is there sometime you notice about these counties?
  * Observation: death rates are high but the actuall number of cases is relatively small:
``` {r}
...
# HELP NEEDED WITH #7 - not sure what 100,000 cases refer to?
...
```{r}
state.level <- aggregate(cbind(cases, deaths)~state, data=latest, FUN=sum)
head(state.level)
```
...
state.level.
```{r}
state.level$death_rate = round((state.level$deaths/ state.level$cases) * 100, 2)
head(state.level)
```
...
```
# HELP NEEDED - PR has unknown death rate as deaths were not reported

e. Which states have the 10 highest death rates?
```{r}
head(state.level[order(-state.level$death_rate), ], 10)
```
