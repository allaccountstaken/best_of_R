...
#### It seems that "Ht" is a better single predictor as it appears to have the strongest correlation with all other variables at the same time.
...
``` {r}
subset_result <- lm(hipcenter ~ Age + Weight + Ht, data=seatpos)
summary(subset_result)
```

#### Yes, multicollinearity is still a possibility, as some slopes are not significant with an overall significant F test.
