...
```{r}
fit <- lm(log(price) ~ . - sqft_living15 , data = house)
summary(fit)
```
...
```{r}
fit_interraction <- lm(log(price) ~ waterfront*sqft_living + . , data = house)
summary(fit_interraction)
```

```{r}
plot(fit_interraction)
```
...
```{r}
regnull <- lm(price ~ 1, data=house)
regfull <- lm(price ~ ., data=house)
...
```{r}
full_model<-glm(value ~ bathrooms+sqft_living+floors+waterfront+view+grade+sqft_living15+age, 
...
# N/A in sqft_basement, 50% of observations have missing values
# Will removing outliers help with Residuals/Fitted values
# How to go from log(price) to 1/0 high value?
